...
INSERT INTO
    tbl_menu
VALUES  -- menu_code 컬럼은 AUTO_INCREMENT 설정이 되어 있으므로 Nuull 처리
    (NULL, '바나나해장국', 15000, 4, 'Y');

SELECT * FROM tbl_menu;

-- insert 하고 싶은 데이터를 컬럼을 지정해서 삽입
INSERT INTO
    tbl_menu (
        menu_name,
        menu_price,
...
-- 컬럼 명시하면 insert시 데이터의 순서를 바꾸는 것도 가능하다.
INSERT INTO
    tbl_menu (
        menu_price,
        menu_name,
...
-- 2. update : 테이블에 기록된 컬럼의 값을 수정하는 구문이다. 테이블의 전체 행의 개수에는 변화가 없다.
...
-- where절에서 삭제 대상 행을 반드시 선택하야 하며 생략할 경우 테이블의 전체 행이 삭제 된다.
DELETE FROM
    tbl_menu
WHERE
    menu_code = 24;

-- oreder by절과 limit절을 활용한 삭제
DELETE FROM
    tbl_menu
ORDER BY
    menu_code DESC
...
-- replace는 내부적으로 기존 행을 삭제 후 다시 삽입해주기 때문에 수정된다.
REPLACE INTO
    tbl_menu
VALUES
    (1,'참기름소주',4500,10,'Y');
