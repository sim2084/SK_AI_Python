...
-- 3. 상관 서브쿼리 (상호 연관 서브쿼리)
-- 메인 쿼리의 현재 행 a.category_code 값을 서브쿼리 내부에서 참조하여 해당 카테고리의 평균가를 구한다.
-- 각 메뉴는 자신이 속한 카테고리의 평균가와 비교되어 그보다 큰 경우만 결과 집합에 담는다.
SELECT
    *
FROM
    tbl_menu a
...
-- 조건이 만족되는 행이 발견되는 순간 바로 true를 반환하므로 대량 데이터에서 존재 여부를 확인할 때 주로 사용한다.
SELECT
    category_code
FROM
    tbl_category a
